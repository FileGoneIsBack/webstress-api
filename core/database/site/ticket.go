...
func (conn *Instance) NewTicket(userID int, title, message, username string) error {
	...
	if err != nil {
		log.Println("database/NewTicket(): error occurred while getting last insert ID:", err)
		return err
	}

	stmtMsg, err := conn.conn.Prepare("INSERT INTO `messages` (`ticket_id`, `user_id`, `message`) VALUES (?, ?, ?)")
	if err != nil {
		log.Println("database/NewTicket(): error occurred while preparing message statement:", err)
		return err
	}
	defer stmtMsg.Close()
	_, err = stmtMsg.Exec(ticketID, userID, message)
	if err != nil {
		log.Println("database/NewTicket(): error occurred while executing message statement:", err)
		return err
	}
...
func (conn *Instance) GetTickets(user *database.User) ([]*Ticket, error) {
	...
	if err != nil {
		log.Println("database/GetTicketsForUser: error occurred while preparing statement:", err)
		return nil, err
	}
	defer stmt.Close()
	...
	if err != nil {
		log.Println("database/GetTicketsForUser: error occurred while executing query:", err)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println("database/GetTicketsForUser: error occurred while scanning row:", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		log.Println("database/GetTicketsForUser: error occurred during iteration:", err)
		return nil, err
	}
...
func (conn *Instance) GetAllTickets() ([]*Ticket, error) {
	...
	if err != nil {
		log.Println("database/GetAllTickets: error occurred while preparing statement:", err)
		return nil, err
	}
	defer stmt.Close()
	...
	rows, err := stmt.Query()
	if err != nil {
		log.Println("database/GetAllTickets: error occurred while executing query:", err)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println("database/GetAllTickets: error occurred while scanning row:", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		log.Println("database/GetAllTickets: error occurred during iteration:", err)
		return nil, err
	}
...
func (conn *Instance) GetTicketByID(ticketID int64) (*Ticket, error) {
	...
	if err != nil {
		log.Println("database/GetTicketByID: error occurred while preparing ticket statement:", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("database/GetTicketByID: error occurred while scanning ticket row:", err)
		return nil, err
	}

	stmtMessages, err := conn.conn.Prepare("SELECT `user_id`, `message`, `created_at` FROM `messages` WHERE `ticket_id` = ?")
	if err != nil {
		log.Println("database/GetTicketByID: error occurred while preparing message statement:", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("database/GetTicketByID: error occurred while querying messages:", err)
		return nil, err
	}
	...
	for rowsMessages.Next() {
		...
		if err != nil {
			log.Println("database/GetTicketByID: error occurred while scanning message row:", err)
			continue
		}
	}

	if err := rowsMessages.Err(); err != nil {
		log.Println("database/GetTicketByID: error occurred during iteration of messages:", err)
		return nil, err
	}
...
func (conn *Instance) UpdateMessage(ticketID int64, userID int, message string) error {
	...
	if err != nil {
		log.Println("database/UpdateMessage: error occurred while preparing statement:", err)
		return err
	}
	defer stmt.Close()
	...
	if err != nil {
		log.Println("database/UpdateMessage: error occurred while executing statement:", err)
		return err
	}
...
func (conn *Instance) GetMessagesByTicketID(ticketID int64) ([]*Message, error) {
	...
	if err != nil {
		log.Println("database/GetMessagesByTicketID: error occurred while preparing statement:", err)
		return nil, err
	}
	defer stmt.Close()
	...
	if err != nil {
		log.Println("database/GetMessagesByTicketID: error occurred while executing query:", err)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println("database/GetMessagesByTicketID: error occurred while scanning row:", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		log.Println("database/GetMessagesByTicketID: error occurred during iteration:", err)
		return nil, err
	}
...
func (conn *Instance) DeleteTicketByID(ticketID int64) error {
	...
	if err != nil {
		log.Println("database/DeleteTicketByID: error occurred while deleting messages:", err)
		return err
	}
	...
	if err != nil {
		log.Println("database/DeleteTicketByID: error occurred while deleting ticket:", err)
		return err
	}
